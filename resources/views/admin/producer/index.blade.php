...
<div class="col-md-12">
    ...
    <div class="card">
        <div class="card-header">
            <h4>Quản lý nhà sản xuất
                <a href="{{url('admin/producer/create')}}" class="btn btn-primary btn-md btn-block float-end">
                    <i class="mdi mdi-home-variant"></i>
                    Thêm nhà sản xuất
                </a>
            </h4>
        </div>
        <form action="{{url('admin/producer/search')}}" method="GET">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6">
        ...
        <div class="card-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @forelse ($producers as $key=>$producer)
                    <tr>
                        <td>{{$key+1}}</td>
                        <td>{{$producer->name}}</td>
                        <td>
                            <a href="{{url('admin/producer/edit/'.$producer->id)}}" class="btn btn-outline-secondary btn-icon-text">
                                Sửa
                                <i class="mdi mdi-file-check btn-icon-append"></i>
                            </a>
                            <a href="{{url('admin/producer/delete/'.$producer->id)}}" onclick="return confirm('Bạn có chắc muốn xóa nhà sản xuất này?')" class="btn btn-outline-danger btn-icon-text">
                                Xóa
                    ...
                    @empty
                    <tr>
                        <td colspan="3">Không có nhà sản xuất nào</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
